...
public class Entity {
  ...
  public void drawSprite(Graphics2D g, int i, int j, int w, int h) {
    ...
    g.drawImage(Entity.spriteMap, new AffineTransform(spriteSize, 0f, 0f, spriteSize,
        (x - (i * spriteSize)), (y - (j * spriteSize))), null);
  ...
  public void drawSprite(Graphics2D g, int size, int i, int j) {
    ...
    g.drawImage(Entity.spriteMap, new AffineTransform(spriteSize, 0f, 0f, spriteSize, x - offset
        - (i * size * spriteSize), y - offset - 1 - (j * size * spriteSize)), null);
    g.setClip(new Rectangle(0, 0, VD.WIDTH, VD.HEIGHT));
  }
  ...
  public void drawSprite(Graphics2D g, int size, int i, int j, int offsetX, int offsetY) {
    ...
    double x = boundingSprite.getX();
    double y = boundingSprite.getY();
    g.clipRect((int) x + offsetX, (int) y + offsetY, (int) (size * spriteSize),
        (int) (size * spriteSize));
    g.drawImage(Entity.spriteMap, new AffineTransform(spriteSize, 0f, 0f, spriteSize, x - offset
        - (i * size * spriteSize) + offsetX, y - offset - 1 - (j * size * spriteSize) + offsetY),
        null);
    g.setClip(new Rectangle(0, 0, VD.WIDTH, VD.HEIGHT));
  }
