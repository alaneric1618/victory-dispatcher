    ...
    switch (type) {
      ...
      case SMOKE:
        ...
        if (!VD.paused) {
          ...
          this.boundingSprite =
              new Rectangle((int) (r.x + speed * Math.cos(dir)),
                  (int) (r.y + speed * Math.sin(dir)), 32, 32);
        }
        drawSprite(g, (34 + (int) tick) * (32), 8 * (32) + 7, 32, 32);
        break;
      case DEBRIS:
        if (wait < 0.0) {
          ...
          if (!VD.paused)
            this.boundingSprite =
                new Rectangle((int) (r2.x + speed * Math.cos(dir)), (int) (r2.y + speed
                    * Math.sin(dir)), 32, 32);
          drawSprite(g, ((int) 82 + (int) ((tick + (2 * speed)) % 8)) * (34) + 10, 8 * (32) + 7,
              31, 32); // 56 X
        }
        break;
      case BLAST:
