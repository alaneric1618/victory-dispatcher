	...
	case SMOKE:
        ...
        case DEBRIS:
            if (wait < 0.0) {
                ...
                if (!VD.paused) this.boundingSprite = new Rectangle((int)(r2.x+speed*Math.cos(dir)), (int)(r2.y+speed*Math.sin(dir)), 32, 32);
                drawSprite(g, ((int)82+(int)((tick+(2*speed))%8))*(34)+10, 8*(32)+7, 31, 32); //56 X
            }
            break;
        case BLAST:
            if (tick < 19) {
                if (!VD.paused) this.boundingSprite = new Rectangle(x-32, y-32, 32, 32);
                drawSprite(g, (34-(int)(tick%18))*(32), 8*(32)+7, 32, 32);
            } 
            if (tick > 12 && tick < 30){
                if (!VD.paused) this.boundingSprite = new Rectangle(x-32+12, y-32+12, 32, 32);
